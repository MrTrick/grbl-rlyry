...
void spindle_init()
{
  // RLYRLY: Configure multi-spindle pins.    
	...
	RLYRLY_DDR &= ~RLYRLY_MASK; //Set each of the spindle outputs to hi-Z
	RLYRLY_PORT &= ~RLYRLY_MASK; // Set all outputs to low, though it won't take effect yet
  #else
...
void spindle_set_state(uint8_t state, spindle_t rpm)
{
  ...
  if (state == SPINDLE_DISABLE) {
	...
	#ifdef RLYRLY_SPINDLE	
		RLYRLY_DDR = (RLYRLY_DDR & ~RLYRLY_MASK) | (rpm << RLYRLY_LSB); //Set each of the spindle outputs to hi-Z / low
	#else  
		#ifndef USE_SPINDLE_DIR_AS_ENABLE_PIN
